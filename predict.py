...
class AIMLDataAnalysis:
    def __init__(self, root):
        ...
        # Title label
        self.title_label = tk.Label(self.frame, text="AIML Data Analysis", bg='grey', font=('Times New Roman', 40, 'bold'), fg='white')
        self.title_label.pack(pady=20)

        # Upload button
        self.upload_button = tk.Button(self.frame, text="Upload CSV File", command=self.upload_csv, font=('Helvetica', 14), height=2, width=20, bg="#4CAF50", fg="white")
        self.upload_button.pack(pady=10)

        # Label to show uploaded file name
        self.file_name_label = tk.Label(self.frame, text="No file selected", bg='grey', font=('Helvetica', 16, 'bold'))
        self.file_name_label.pack(pady=10)

        # Predict button
        self.predict_button = tk.Button(self.frame, text="Predict", command=self.run_prediction, font=('Helvetica', 14), height=2, width=15, bg="#2196F3", fg="white")
        self.predict_button.pack(pady=20)

        # Return button
        self.return_button = tk.Button(root, text="Return", command=self.return_to_previous, fg="white", bg="red", font=('Helvetica', 14), height=2, width=10)
    ...
    def upload_csv(self):
        ...
        try:
            ...
            # Show months uploaded instead of file name
            months_uploaded = self.data['entry_date'].apply(lambda x: pd.to_datetime(x, format='%d-%m-%Y', errors='coerce')).dropna()
            if months_uploaded.empty:
                self.file_name_label.config(text="Uploaded File: No valid dates found")
            else:
                months_list = months_uploaded.dt.strftime('%B %Y').unique()
                months_str = ", ".join(months_list)
                self.file_name_label.config(text=f"Months Uploaded: {months_str}")
    ...
    def run_prediction(self):
        ...
        try:
            results = self.predict_with_polynomial_regression(self.data)
            self.display_results(*results)
        except Exception as e:
            messagebox.showerror("Prediction Error", f"An error occurred: {str(e)}")

    def predict_with_polynomial_regression(self, data):
        if 'entry_date' not in data.columns or 'Total_Carbon_Footprint_kgCO2' not in data.columns:
            raise ValueError("CSV must have 'entry_date' and 'Total_Carbon_Footprint_kgCO2' columns. Consider renaming the columns to 'entry_date' and 'Total_Carbon_Footprint_kgCO2'.")
        ...
        X = data[['Days']].values
        y = data['Total_Carbon_Footprint_kgCO2'].values
        ...
        # Ensure non-negative predictions (set negative values to zero)
        future_forecast[future_forecast < 0] = 0

        # Handle infinite or very large values
        future_forecast = np.clip(future_forecast, 0, np.inf)
    ...
    def display_results(self, monthly_avg_forecast, yearly_total_forecast, next_month_df):
        ...
        # Add original data monthly summary tab
        original_monthly = self.data.groupby(self.data['entry_date'].dt.to_period('M'))['Total_Carbon_Footprint_kgCO2'].sum().reset_index()
        original_monthly['entry_date'] = original_monthly['entry_date'].dt.to_timestamp()
        self.add_tab(notebook, "Original Monthly Data", original_monthly)
        ...
        download_button = tk.Button(result_window, text="Download Report", command=lambda: self.download_report(original_monthly, monthly_avg_forecast, yearly_total_forecast, next_month_df), font=("Helvetica", 14), bg="#FF9800", fg="white")
    ...
    def download_report(self, original_monthly, monthly_avg_forecast, yearly_total_forecast, next_month_df):
        ...
        try:
            if file_path.endswith(".csv"):
                original_monthly.to_csv(file_path, index=False)
                monthly_avg_forecast.to_csv(file_path, index=False, mode='a', header=True)
                ...
                with open(file_path, "w") as f:
                    f.write("Original Monthly Data:\n")
                    f.write(original_monthly.to_string(index=False))
                    f.write("\n\nMonthly Predictions:\n")
